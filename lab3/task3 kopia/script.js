...
import { infoVehicle, rentVehicle, buyVehicle, returnVehicle, unrentAllVehicles } from './appActions.js'
const DB_SIZE = 50;
let dbItems = [];
        ...
        for (const vehicle of Object.values(dbItems)) {
            ...
            if (vehicle.rent_by === null && vehicle.bought_by === null && typeFilterSet.includes(vehicle.type)) {
                ...
                function ActionButton(type, id) {
                    ...
                    return <button actiontype={mode} vehicleid={id} type={`button`} className={`btn btn-light`} data-bs-toggle={`modal`} data-bs-target={`#exampleModal`} style={{ width: '45%' }}>
                        {text}
                    </button>
                }
